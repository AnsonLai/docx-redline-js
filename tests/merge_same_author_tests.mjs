...
// ---------------------------------------------------------------------------
// 4. Re-editing a wholly inserted paragraph preserves its paragraph revision
// ---------------------------------------------------------------------------
{
    const initialDoc = createDocumentXml(`
    ...
    `);
    const firstTurn = await applyOperationToDocumentXml(initialDoc, {
        type: 'redline',
        target: 'Original heading',
        modified: 'Inserted heading\n\nInserted explanation\n\nOriginal heading'
    }, 'Alice');
    assert.equal(firstTurn.hasChanges, true);

    const secondTurn = await applyOperationToDocumentXml(firstTurn.documentXml, {
        type: 'redline',
        target: 'Inserted heading',
        modified: 'Revised inserted heading'
    }, 'Alice');
    assert.equal(secondTurn.status, 'ok');
    assert.equal(secondTurn.hasChanges, true);
    assert.equal(secondTurn.error, undefined);

    const accepted = acceptTrackedChangesInOoxml(secondTurn.documentXml, { author: 'Alice' });
    assert.match(ingestWordOoxmlToPlainText(accepted.oxml), /Revised inserted heading/);
    const rejected = rejectTrackedChangesInOoxml(secondTurn.documentXml, { author: 'Alice' });
    assert.equal(
        ingestWordOoxmlToPlainText(rejected.oxml).replace(/\s+/g, ' ').trim(),
        'Original heading Tail paragraph',
        'rejecting a revised inserted paragraph must restore the pre-insertion document'
    );
}

// Paragraph formatting on a freshly inserted paragraph must not clone its
// paragraph-mark revision into w:pPrChange with the same revision ID.
{
    const initialDoc = createDocumentXml(`
        <w:p><w:pPr><w:jc w:val="left"/></w:pPr><w:r><w:t>Original heading</w:t></w:r></w:p>
        <w:p><w:r><w:t>Tail paragraph</w:t></w:r></w:p>
    `);
    const result = await applyOperationsToDocumentXml(initialDoc, [
        {
            type: 'redline',
            target: 'Original heading',
            modified: 'Inserted heading\n\nInserted explanation\n\nOriginal heading'
        },
        {
            ...
            target: 'Inserted explanation',
            properties: { alignment: 'both' }
        }
    ], 'Alice');
    assert.deepEqual(result.results.map(item => item.status), ['applied', 'applied']);
    const generatedErrors = validateRedlineOoxml(result.documentXml).issues
        .filter(issue => issue.severity === 'error');
    assert.deepEqual(generatedErrors, []);
}

// The same empty-baseline merge path is used for paragraphs generated by
// structured native-list reconstruction. A later edit must not parse an empty
// fragment or discard the original list item's rejection baseline.
{
    const initialDoc = createDocumentXml(`
        <w:p><w:r><w:t>1. Original list item</w:t></w:r></w:p>
        <w:p><w:r><w:t>Tail paragraph</w:t></w:r></w:p>
    `);
    const firstTurn = await applyOperationToDocumentXml(initialDoc, {
        type: 'redline',
        target: '1. Original list item',
        modified: '1. Inserted list item\n\nInserted explanation\n\n2. Original list item'
    }, 'Alice');
    assert.equal(firstTurn.hasChanges, true);

    // Native list labels are computed, so the exact paragraph text excludes
    // the visible "1." label.
    const secondTurn = await applyOperationToDocumentXml(firstTurn.documentXml, {
        type: 'redline',
        target: 'Inserted list item',
        modified: 'Revised inserted list item'
    }, 'Alice');
    assert.equal(secondTurn.status, 'ok');
    assert.equal(secondTurn.hasChanges, true);
    assert.notEqual(secondTurn.error?.code, 'PARSE_ERROR');

    const accepted = acceptTrackedChangesInOoxml(secondTurn.documentXml, { author: 'Alice' });
    assert.match(ingestWordOoxmlToPlainText(accepted.oxml), /Revised inserted list item/);
    const rejected = rejectTrackedChangesInOoxml(secondTurn.documentXml, { author: 'Alice' });
    assert.equal(
        ingestWordOoxmlToPlainText(rejected.oxml).replace(/\s+/g, ' ').trim(),
        '1. Original list item Tail paragraph'
    );
}

// Character and paragraph formatting may follow a structural redline in the
// same live-DOM batch. All revision IDs must remain globally unique.
{
    const initialDoc = createDocumentXml(`
        <w:p><w:pPr><w:jc w:val="left"/></w:pPr><w:r><w:t>Original heading</w:t></w:r></w:p>
        <w:p><w:r><w:t>Tail paragraph</w:t></w:r></w:p>
    ...
    const outputDoc = new DOMParser().parseFromString(result.documentXml, 'application/xml');
    const revisionIds = Array.from(outputDoc.getElementsByTagName('*'))
        .filter(node => ['ins', 'del', 'rPrChange', 'pPrChange'].includes(node.localName))
        .map(node => node.getAttribute('w:id') || node.getAttributeNS(NS_W, 'id'))
        .filter(Boolean);
    assert.equal(new Set(revisionIds).size, revisionIds.length, 'revision IDs must be unique');
    assert.deepEqual(
        validateRedlineOoxml(result.documentXml).issues.filter(issue => issue.severity === 'error'),
        []
    );
}

// ---------------------------------------------------------------------------
// 5. Mixed authors in target paragraph protection
// ---------------------------------------------------------------------------
{
...
// ---------------------------------------------------------------------------
// 6. Batch operation with multiple edits to same paragraph in single run
// ---------------------------------------------------------------------------
{
    const initialDoc = createDocumentXml(`
        <w:p w:paraId="P1">
...
// ---------------------------------------------------------------------------
// 7. Same-author merging refuses commented revision content
// ---------------------------------------------------------------------------
{
...
// ---------------------------------------------------------------------------
// 8. Empty accepted-view paragraphs cannot bypass third-party protection
// ---------------------------------------------------------------------------
{
...
// ---------------------------------------------------------------------------
// 9. Property revisions participate in author detection and protection
// ---------------------------------------------------------------------------
{
