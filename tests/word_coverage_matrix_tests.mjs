...
assert.equal(cases.filter(item => item.lane === 'synthetic').length, 49);
...
assert.equal(report.totals.aiPreflight, 24);
assert.equal(report.totals.syntheticWord, 49);
...
assert.ok(review.cases.length >= Math.ceil(49 * 0.2) + 2);
